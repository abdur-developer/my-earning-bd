...
<body>
  <div class="container">
    ...
    if (isset($_REQUEST['id_w'])) {
      $id = $_REQUEST['id_w'];

      $sql = "SELECT * FROM withdrew WHERE id = '$id'";
      ...
      <form method="get" action="">
        ...
        <div class="input-group">
          <label>Amount:</label>
          <input type="number" name="balance" value="<?php echo $amount; ?>" readonly class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        </div>
  ...
  <?php } else {
      if (isset($_REQUEST['status'])) {
        ...
        $sql = "UPDATE withdrew SET status = '$st' WHERE id = '$id'";
        $query = mysqli_query($conn, $sql);
        if ($query) { ?>
